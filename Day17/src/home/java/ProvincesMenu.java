...
public class ProvincesMenu {
    public static void main(String[] args) {
        ...
        HashMap<Integer,String> provinces = new HashMap<>(temp.length);

        LinkedHashMap<String, ArrayList<String>> result = new LinkedHashMap<>();

        for (int i = 0; i < temp.length; i++) {
            String[] proCity = temp[i].split(":");
            //System.out.println(proCity[0]+"\t"+proCity[1]);
            provinces.put(i+1,proCity[0]);


            ArrayList<String> cites = new ArrayList<>();

            String[] cityStrings = proCity[1].split(",");

            for (int j = 0; j < cityStrings.length; j++) {
                cites.add(cityStrings[j]);
            }

            result.put(proCity[0],cites);
        }

        int choice = scanner.nextInt();

        System.out.print("该省份共有辖区："+result.get(provinces.get(choice)));
    }
}
